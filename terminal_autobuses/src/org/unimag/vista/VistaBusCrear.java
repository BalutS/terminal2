...
import org.unimag.controlador.BusControladorGrabar;
...
public class VistaBusCrear extends StackPane {
    ...
    private ComboBox<String> cbmEmpresa;
    ...
    private void crearFormulario() {
        ...
        txtModeloBus.setPromptText("Digita el modelo del bus");
        GridPane.setHgrow(txtModeloBus, Priority.ALWAYS);
        txtModeloBus.setPrefHeight(ALTO_CAJA);
        miGrilla.add(txtModeloBus, 1, 2);

        Label lblEmpresa = new Label("Empresa del Bus");
        lblEmpresa.setFont(Font.font("Times New Roman", FontPosture.ITALIC, TAMANIO_FUENTE));
        miGrilla.add(lblEmpresa, 0, 3);

        cbmEmpresa = new ComboBox<>();
        cbmEmpresa.setMaxWidth(Double.MAX_VALUE);
        cbmEmpresa.setPrefHeight(ALTO_CAJA);
        cbmEmpresa.getItems().add("Selecciona una empresa");
        EmpresaServicio empresaServicio = new EmpresaServicio();
        empresas = empresaServicio.selectFrom();
        for (EmpresaDto empresa : empresas) {
            cbmEmpresa.getItems().add(empresa.getNombreEmpresa());
        }
        cbmEmpresa.getSelectionModel().select(0);
        miGrilla.add(cbmEmpresa, 1, 3);
    ...
    private Boolean formularioCompleto() {
        ...
        if (cbmEmpresa.getSelectionModel().getSelectedIndex() == 0) {
            Mensaje.mostrar(Alert.AlertType.WARNING, null, "Alerta", "Escoge una empresa");
            cbmEmpresa.requestFocus();
            return false;
        }
    ...
    private void guardarBus() {
        ...
        if (formularioCompleto()) {
            ...
            int index = cbmEmpresa.getSelectionModel().getSelectedIndex() - 1;
            dto.setIdEmpresa(empresas.get(index).getIdEmpresa());
